...
def split_text_into_sentences(text: str) -> list[str]:
    ...
    for s in raw_sentences:
        ...
        if not trimmed:
            continue
        # Nếu câu quá dài (> 100 ký tự), tiếp tục tách ở dấu chấm phẩy (không tách ở dấu phẩy)
        if len(trimmed) > 100:
    ...
    # Gộp các câu quá ngắn (ví dụ: < 15 ký tự) để tránh câu quá ngắn
    merged_sentences = []
    min_len = 15
    for s in sentences:
        if not merged_sentences:
            ...
            if len(merged_sentences[-1]) < min_len:
...
def start_live():
    ...
    env['LLM_API_BASE_URL'] = llm_url
    env['OLLAMA_BASE_URL'] = llm_url
